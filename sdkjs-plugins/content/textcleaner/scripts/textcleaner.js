    ...
    const specialHandlers = {
        removeBgOutline: () => {
            callCommand(() => {
                ...
                // Seçili metin varsa sadece o bölüme uygula
                if (range && range.GetText && range.GetText() !== "") {
                    ...
                    // Seçili metin yoksa tüm dokümanı işle
                    const paragraphs = doc.GetAllParagraphs();
                    for (let i = 0; i < paragraphs.length; i++) {
                        ...
                        const paraPr = para.GetParaPr && para.GetParaPr();
                        if (paraPr) {
        ...
        textCaseConversion: caseOption => {
            ...
            callCommand(() => {
                ...
                // Seçili metin varsa sadece seçili kısmı değiştir
                if (range && range.GetText && range.GetText().trim() !== "") {
                    ...
                    if (convertedText !== selectedText) {
                        // Seçili metni direkt değiştir
                        doc.GetRangeBySelect().SetText(convertedText);
                    }
                } else {
                    // Seçili metin yoksa tüm dokümanı işle
                    const paragraphs = doc.GetAllParagraphs();
